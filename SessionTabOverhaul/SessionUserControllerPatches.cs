...
namespace SessionTabOverhaul
{
    ...
    internal static class SessionUserControllerPatches
    {
        ...
        private static string GetUserDeviceLabel(FrooxEngine.User user) => (user.VR_Active && user.HeadDevice != HeadOutputDevice.Headless && !user.IsPresentInHeadset) ? $"<s>{GetUserDevice(user)}" : GetUserDevice(user);
        ...
        private static VoiceMode GetUserVoiceMode(FrooxEngine.User user) => user.isMuted ? VoiceMode.Mute : user.VoiceMode;

        private static string GetUserVoiceModeLabel(FrooxEngine.User user) => GetUserVoiceMode(user) switch
        {
        ...
        private static colorX GetUserVoiceModeColor(FrooxEngine.User user) => VoiceHelper.GetColor(GetUserVoiceMode(user)).SetSaturation(.5f);

        private static string GetUserFPSOrQueuedMessages(FrooxEngine.User user) => user.QueuedMessages > 10 ? $"<color={queuedMessagesColor}>{user.QueuedMessages} <size=60%>Q'd" : $"<color=#F0F0F0>{MathX.RoundToInt(user.FPS)} <size=60%>FPS";
